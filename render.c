...
void blit_eqin_text(TTF_Font *font, int *const text_size) {
    if (eqin_text.size) {
        SDL_Surface *eqin_text_surface = SDL_scp(TTF_RenderText_Blended_Wrapped(
            font, eqin_text.data, FONT_COLOR, 0));

        *text_size = eqin_text_surface->w;

        SDL_Rect eqin_text_rect = {
            0, 0, MIN(eqin_text_surface->w, BG_EQIN_TEXT_WIDTH),
            eqin_text_surface->h};
        ...
        SDL_scc(SDL_BlitSurface(eqin_text_surface, &eqin_text_rect,
                                eqin_surface, &bg_eqin_text_rect));
...
void blit_eqin_scroll(int text_size) {
    if (text_size > BG_EQIN_TEXT_WIDTH && text_size) {
        ...
        eqin_scroll_surface = SDL_scp(SDL_CreateRGBSurface(
            0, bg_eqin_scroll_rect.w, bg_eqin_scroll_rect.h, 32, 0, 0, 0, 0));
        ...
        SDL_scc(SDL_BlitSurface(eqin_scroll_surface, &eqin_scroll_rect,
                                eqin_surface, &bg_eqin_scroll_rect));

        SDL_FreeSurface(eqin_scroll_surface);
        eqin_scroll_surface = NULL;
        //     printf("eqin_scroll_rect: %d, %d, %d, %d\t", eqin_scroll_rect.x,
        //     eqin_scroll_rect.y,
        //            eqin_scroll_rect.w, eqin_scroll_rect.h);
        //     printf("bg_eqin_scroll_rect: %d, %d, %d, %d\n",
        //     bg_eqin_scroll_rect.x, bg_eqin_scroll_rect.y,
        //            bg_eqin_scroll_rect.w, bg_eqin_scroll_rect.h);
    }
}

void render_eqin(SDL_Renderer *renderer, TTF_Font *font) {
    eqin_surface = SDL_scp(
        SDL_CreateRGBSurface(0, eqin_rect.w, eqin_rect.h, 32, 0, 0, 0, 0));
    SDL_scc(
        SDL_FillRect(eqin_surface, NULL,
